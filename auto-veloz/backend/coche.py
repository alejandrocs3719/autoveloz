...
class Coche:
    ...
    @staticmethod
    def obtener_desde_db(matricula : str | None) -> tuple | list[tuple]:
        conexion = psycopg2.connect(
            dbname="postgres",
            user="postgres",
            password="DhYae8vU820ZZec7AW5S",
        ...
        coche_data : tuple | list[tuple]
        coche      : Coche | list[Coche] | None 
        ...
        if matricula is not None:
            ...
            if len(coche_data) != 0:
                coche = [Coche(*tup[1:]) for tup in coche_data]
            else:
                coche = None
    ...
    def coche_en_X_oficina(req: CocheEnXOficinaRequest):
        ...
        cursor.execute( #TODO poner req.marca
            ...
            AND wifi = %(wifi)s
            AND gps = %(gps)s
            AND silla_nino = %(silla_nino)s
            AND cadenas = %(cadenas)s;
            """
        )
        coche_data = cursor.fetchall()
        cursor.close()
        ...
        if coche_data:
            for coche in coche_data:
                
                oficinas_set.add(Coche(**coche).id_oficina_actual)
