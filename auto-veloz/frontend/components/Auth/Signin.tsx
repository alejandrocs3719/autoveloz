...
const Signin = () => {
  ...
  const handleSignin = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("http://13.48.84.201:8000/login", {
        method: "POST",
      ...
      if (!res.ok || Object.keys(errorData).length === 0) {
        console.log("API error response body:", errorData);
        const errorMessage =
          errorData.detail ||
          (Object.keys(errorData).length === 0
            ? "Credenciales inválidas"
            : `Error en el login (status: ${res.status})`);
        throw new Error(errorMessage);
      }

      // Respuesta válida
      const userData: User = errorData;
      ...
      router.push(userData.tipo_usuario === "admin" ? "/admin/usuarios" : "/");
    } catch (err: any) {
          ...
          >
            ...
            <form onSubmit={handleSignin}>
              ...
              <div className="flex flex-wrap items-center gap-10 md:justify-between xl:gap-15">
                ...
                <div className="flex flex-wrap gap-4 md:gap-10">{/* ... */}</div>
                <button
                  type="submit"
                  disabled={loading}
